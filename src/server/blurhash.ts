/**
 * Turning a stored blurhash into something the browser can paint.
 *
 * Decode-side only, and deliberately so: this file is imported by
 * `imageData.ts` and therefore runs inside the Cloudflare Worker, where there
 * is no filesystem and no WASM to load off disk. The *encoder* — which needs to
 * decode a real image first — lives in `src/image/blurhash.ts` and runs only on
 * Trigger.dev.
 *
 * The PNG writer below is hand-rolled for the same reason. It emits stored
 * (uncompressed) deflate blocks, which is legal PNG and costs nothing to
 * produce; at 32x32 the output is a few KB inlined as a data URL, so there is
 * no compression worth the code to do it.
 */
