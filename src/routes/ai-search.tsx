...
export const Route = createFileRoute("/ai-search")({
  ...
  loaderDeps: ({ search: { q, model, reranker, showScore, rrfConfig } }) => ({
    q,
    model,
    reranker,
    showScore,
  ...
  loader: async ({ deps: data }) => {
    ...
    let searchModel:
      | string
      | { model: string; k: number; weight: number }[]
      | undefined = data.model;
    if (data.model === "rrf" && data.rrfConfig) {
      try {
        const parsed = JSON.parse(data.rrfConfig) as {
          model: string;
          k: number;
          weight: number;
        }[];
...
function RouteComponent() {
  ...
  const [rrfSlots, setRrfSlots] = useState<RRFSlot[]>(() =>
    parseRrfConfig(rrfConfig),
  );
  ...
  const updateSlot = (
    index: number,
    updates: Partial<NonNullable<RRFSlot>>,
  ) => {
    setRrfSlots((prev) => {
      const next = [...prev];
          ...
          <>
            ...
            {selectedModel === "rrf" && (
              <div className="flex flex-col gap-2 mt-1">
                <div
                  className="grid text-xs text-gray-500 font-medium"
                  style={{ gridTemplateColumns: "1fr 80px 70px 60px" }}
                >
                  <span>Model</span>
                  <span>Weight</span>
                  <span>K</span>
                ...
                {rrfSlots.map((slot, i) => (
                  ...
                  >
                    ...
                    <input
                      ...
                      disabled={!slot}
                      onChange={(e) =>
                        updateSlot(i, { k: parseInt(e.target.value) || 60 })
                      }
                    />
                    <button
                      type="button"
