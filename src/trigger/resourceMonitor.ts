...
export class ResourceMonitor {
  ...
  startMonitoring(intervalMs = 10000): void {
    if (intervalMs < 1000) {
      intervalMs = 1000;
      this.logger.warn(
        "ResourceMonitor: intervalMs is less than 1000, setting to 1000",
      );
    }
  ...
  private async getProcessMetrics(): Promise<ProcessMetrics> {
    ...
    const nodeMetrics: NodeProcessMetrics = {
      ...
      isNearHeapLimit:
        heapStats.used_heap_size / heapStats.heap_size_limit > 0.8,
    };
    ...
    try {
      ...
      const memoryScaleFactor = this.ctx.machine
        ? totalMemory / machineMemoryBytes
        : 1;
  ...
  private createEnhancedLabel(payload: any, baseLabel: string): string {
    ...
    parts.push(
      `Node:${nodeMemPercent.toFixed(1).padStart(4)}%${nodeIndicator}`,
    );
    ...
    if (
      ...
      payload.process.targetProcess.count > 0
    ) {
      const targetCpu = payload.process.targetProcess.totals?.cpuPercent || "0";
      const targetMem =
        payload.process.targetProcess.totals?.memoryPercent || "0";
    ...
    if (payload.constraints) {
      parts.push(
        `[${payload.constraints.cpu}CPU/${payload.constraints.memoryGB}GB]`,
      );
    }
  ...
  private createCompactLabel(payload: any, baseLabel: string): string {
    ...
    const memIndicator =
      memoryPercent > 95 ? "!" : memoryPercent > 80 ? "?" : ".";
    ...
    if (payload.gc && payload.gc.count > 0 && payload.gc.avgDuration > 2) {
      const gcIndicator =
        payload.gc.avgDuration > 10
          ? "!"
          : payload.gc.avgDuration > 5
            ? "?"
            : ".";
    ...
    return {
      ...
      process: {
        node: {
          ...
          memoryUsagePercent: formatPercent(
            processMetrics.node.memoryUsagePercent,
          ),
        ...
        targetProcess: processMetrics.targetProcess
          ? {
              ...
              averages: processMetrics.targetProcess.averages
                ? {
                    cpuPercent: formatPercent(
                      processMetrics.targetProcess.averages.cpu * 100,
                    ),
                    memoryPercent: formatPercent(
                      processMetrics.targetProcess.averages.memory,
                    ),
                    rssMB: formatBytes(
                      processMetrics.targetProcess.averages.rss * 1024,
                    ),
                    vszMB: formatBytes(
                      processMetrics.targetProcess.averages.vsz * 1024,
                    ),
                  }
                : null,
              totals: processMetrics.targetProcess.totals
                ? {
                    cpuPercent: formatPercent(
                      processMetrics.targetProcess.totals.cpu * 100,
                    ),
                    memoryPercent: formatPercent(
                      processMetrics.targetProcess.totals.memory,
                    ),
                    rssMB: formatBytes(
                      processMetrics.targetProcess.totals.rss * 1024,
                    ),
                    vszMB: formatBytes(
                      processMetrics.targetProcess.totals.vsz * 1024,
                    ),
                  }
                : null,
            }
