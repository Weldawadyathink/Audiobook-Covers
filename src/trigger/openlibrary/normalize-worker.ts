...
export const openLibraryNormalizeWorkerTask = schemaTask({
  id: "openlibrary-normalize-worker",
  machine: "small-2x",
  retry: {
    maxAttempts: 3,
    outOfMemory: {
  ...
  run: async ({ source, target, limit, offset, queryToUse }, { ctx }) => {
    const db = await setupDuckDB(ctx);
